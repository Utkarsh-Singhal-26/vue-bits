import code from '@/content/Components/DriftWall/DriftWall.vue?raw';
import { createCodeObject } from '@/types/code';

export const driftWall = createCodeObject(code, 'Components/DriftWall', {
  usage: `<script setup>
import DriftWall from './DriftWall.vue'

const items = [
  { image: 'https://picsum.photos/id/1015/600/400', title: 'Peaks', href: 'https://example.com/one' },
  { image: 'https://picsum.photos/id/1025/600/400', title: 'Pup', href: 'https://example.com/two' },
  { image: 'https://picsum.photos/id/1039/600/400', title: 'Falls', href: 'https://example.com/three' }
];
</script>

<template>
  <div :style="{ height: '600px' }">
    <DriftWall
      :items="items"
      :columns="5"
      :tileWidth="200"
      :tileHeight="132"
      :gap="18"
      :tilt="16"
      :turn="-14"
      :perspective="1200"
      :depth="120"
      :speed="42"
      direction="up"
      :variance="0.45"
      :parallax="0.6"
      :lift="64"
      :fade="0.6"
      :dim="0.55"
      overlayColor="#060010"
    />
  </div>
</template>`
});
