...
interface MainUniforms {
  ...
  uColumns: ScalarUniform;
  uRows: ScalarUniform;
  uThickness: ScalarUniform;
  uSpeed: ScalarUniform;
  uTravel: ScalarUniform;
  uWaveSpread: ScalarUniform;
  uRowOffset: ScalarUniform;
  uSoftness: ScalarUniform;
  uGlow: ScalarUniform;
  uBrightness: ScalarUniform;
  uContrast: ScalarUniform;
  uOpacity: ScalarUniform;
  uVertical: ScalarUniform;
  uAlternate: ScalarUniform;
  uMouse: Vec2Uniform;
...
const props = withDefaults(defineProps<SlicedWavesProps>(), {
  color1: '#FF9FFC',
  color2: '#5227FF',
  color3: '#B497CF',
  columns: 14,
...
const syncUniforms = () => {
  ...
  u.uColumns.value = Math.max(1, Math.round(props.columns));
  u.uRows.value = Math.max(1, Math.round(props.rows));
  u.uThickness.value = props.barThickness;
  u.uSpeed.value = props.speed;
  u.uTravel.value = props.travel;
  u.uWaveSpread.value = props.waveSpread;
  u.uRowOffset.value = props.rowOffset;
  ...
  u.uVertical.value = props.orientation === 'vertical' ? 1.0 : 0.0;
  u.uAlternate.value = props.alternate ? 1.0 : 0.0;
  u.uMouseStrength.value = props.mouseStrength;
  u.uMouseRadius.value = props.mouseRadius;
  u.uEnableMouse.value = props.mouseInteraction ? 1.0 : 0.0;
...
onMounted(() => {
  ...
  const loop = (t: number) => {
    ...
    mu.uMouseActive.value = currentActive;
    renderer.render({ scene: mesh });
