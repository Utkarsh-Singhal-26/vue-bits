...
const waitForFonts = async (font: string): Promise<void> => {
  ...
  try {
    ...
    /* font load failed; proceed with whatever is available */
  }

  await document.fonts.ready;
};

const props = withDefaults(defineProps<ParticleTextProps>(), {
  text: 'React Bits',
  particleSize: 2,
  density: 4,
  color: '#ffffff',
...
const setup = () => {
  ...
  const render = (now: number): void => {
    ...
    if (props.glow && !reducedMotion) {
      ctx.shadowBlur = props.particleSize * 3;
  ...
  const sampleText = async (): Promise<void> => {
    ...
    let resolvedSize = resolveFontSize(props.fontSize, container, props.fontWeight, resolvedFamily);
    ...
    particles = selected.map((target, index) => {
      ...
      const particleColor = baseRgb && highlightRgb ? rgbToCss(mixRgb(baseRgb, highlightRgb, blend)) : props.color;
...
<template>
  <div ref="containerRef" class="block relative w-full h-full min-h-60 overflow-hidden touch-none" :aria-label="text">
    <canvas ref="canvasRef" class="block absolute inset-0 w-full h-full" aria-hidden="true" />
