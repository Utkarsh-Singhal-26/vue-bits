...
const props = withDefaults(defineProps<ElasticMeshProps>(), {
  image: '',
  color1: '#10B981',
  color2: '#c6d0b4',
  highlight: '#ffffff',
...
const setup = () => {
  ...
  const N = Math.max(6, Math.min(40, Math.round(props.resolution)));
  const nodeCount = N * N;
  ...
  function toPlane(clientX: number, clientY: number) {
    ...
    const clipX = mx * 2 - 1;
    const clipY = 1 - my * 2;
    const angle = (props.tilt * Math.PI) / 180;
...
<template>
  <div ref="containerRef" class="relative w-full h-full" :style="{ touchAction: 'none' }" />
</template>
