...
interface VecArrUniform {
  value: number[];
}
interface TextureUniform {
  value: InstanceType<typeof RenderTarget>['texture'] | null;
}
...
interface CompUniforms {
  tField: TextureUniform;
  uColor: VecArrUniform;
  uAccent: VecArrUniform;
  uMerge: ScalarUniform;
  uGlow: ScalarUniform;
  uOpacity: ScalarUniform;
}

const props = withDefaults(defineProps<SwarmCursorProps>(), {
  color: '#ffffff',
  accentColor: '#ffffff',
  count: 10,
  size: 10,
...
onMounted(() => {
  ...
  const fu = fieldProgram.uniforms as unknown as FieldUniforms;
  ...
  const compProgram = new Program(gl, {
    ...
    uniforms: {
      ...
      uAccent: { value: hexToRgb(props.accentColor) },
      uMerge: { value: props.merge },
  ...
  const cu = compProgram.uniforms as unknown as CompUniforms;
  ...
  let burst = 0;
  let activeCount = Math.max(1, Math.min(MAX, Math.round(props.count)));
  ...
  const onDown = (e: PointerEvent) => {
    ...
    const escape = 620 + props.speed * 130;
    for (let i = 0; i < MAX; i++) {
      let dx = px[i] - cx;
      let dy = py[i] - cy;
  ...
  const frame = (now: number) => {
    ...
    cu.uColor.value = hexToRgb(p.color);
    cu.uAccent.value = hexToRgb(p.accentColor);
    cu.uMerge.value = p.merge;
    cu.uGlow.value = p.glow;
    cu.uOpacity.value = p.opacity;

    renderer.render({ scene: fieldMesh, target, clear: true });
    cu.tField.value = target.texture;
    renderer.render({ scene: compMesh });
  };
  raf = requestAnimationFrame(frame);
...
<template>
  <div ref="containerRef" class="relative w-full h-full" :class="className" :style="style">
    <div v-if="$slots.default" class="absolute inset-0 flex justify-center items-center pointer-events-none">
      <slot />
    </div>
  </div>
</template>
