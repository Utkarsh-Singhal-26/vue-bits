...
const syncUniforms = () => {
  if (!uniforms) return;
  const u = uniforms;
  u.wave.uRings.value = props.rings;
  u.composite.uStrength.value = props.strength;
  u.composite.uSwirl.value = props.swirl;
  u.composite.uDispersion.value = props.dispersion;
  u.composite.uGlint.value = props.glint;
  u.composite.uTintAmount.value = props.tintAmount;
  u.composite.uGrayscale.value = props.grayscale ? 1 : 0;
  u.composite.uHighlight.value = hexToRGB(props.highlightColor);
  u.composite.uTint.value = hexToRGB(props.tint);
};
...
const setup = () => {
  ...
  const resize = () => {
    ...
    const scale = QUALITY_SCALE[props.quality] || QUALITY_SCALE.high;
  ...
  const onMove = (event: PointerEvent) => {
    if (!props.enabled || reduceMotion || props.trigger === 'click') return;
  ...
  const onDown = (event: PointerEvent) => {
    if (!props.enabled || reduceMotion || props.trigger === 'hover') return;
    const point = localPoint(event.clientX, event.clientY);
    if (!point) return;
  ...
  const loop = (now: number) => {
    ...
    const decay = reduceMotion ? 1 : Math.exp((-delta * LIFE_CONSTANT) / Math.max(0.15, props.fade));
...
<template>
  <div
    ref="mountRef"
    class="[&>canvas]:block relative bg-black w-full [&>canvas]:w-full h-full [&>canvas]:h-full overflow-hidden"
    :class="className"
    :style="style"
  />
</template>
